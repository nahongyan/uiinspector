...
void Server::setUrl(const QUrl &newUrl)
{
    ...
    m_serverDevice = ServerDevice::create(m_url, this);
    if (!m_serverDevice)
        return;

    if (auto *localServerDevice = qobject_cast<LocalServerDevice*>(m_serverDevice)) {
        localServerDevice->setServerAddress(m_url);
    }
    ...
    connect(m_signalMapper, &MultiSignalMapper::signalEmitted, this, &Server::forwardSignal);
}
...
{
    qDebug() << "invokeObject" << objectName << method << args.size() ;
