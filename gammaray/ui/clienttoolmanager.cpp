...
static void initPluginRepository()
{
    ...
    insertFactory(new WidgetInspectorUiFactory);
    insertFactory(new GuiSupportUiFactory);
...
void ClientToolManager::gotTools(const QVector<GammaRay::ToolData> &tools)
{
    ...
    QSet<QString> reportedIds;
    for (const auto &tool : tools) {
        reportedIds.insert(tool.id);
        ToolUiFactory *factory = s_pluginRepository()->factories.value(tool.id);
        if (factory) {
    ...
    for (auto it = s_pluginRepository()->factories.constBegin(); it != s_pluginRepository()->factories.constEnd(); ++it) {
        if (!reportedIds.contains(it.key())) {
            ToolData td;
            td.id = it.key();
            td.hasUi = true;
            td.enabled = true;
            m_tools.append(ToolInfo(td, it.value()));
        }
    }
