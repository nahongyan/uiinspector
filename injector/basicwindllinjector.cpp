/*
  basicwindllinjector.cpp - self-contained copy for standalone build
*/
...
{
    return remoteKernel32Call(destProcess, "LoadLibraryW", dllPath);
}

BOOL BasicWinDllInjector::injectProcess(int pid, wchar_t *path, wchar_t *probePath)
{
    HANDLE destProcess = OpenProcess(PROCESS_ALL_ACCESS, false, pid);
    if (!destProcess) {
        std::wstringstream stream;
        stream << L"Failed to open process" << pid;
        log(stream.str().c_str());
        BasicWinDllInjector::logError();
        return FALSE;
    }
